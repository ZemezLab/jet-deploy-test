					...
					props.isSelected && (
						...
						>
							...
							>
								...
								{ 'text' === attributes.marker_type && filterCallbacksArgs.length && filterCallbacksArgs.map( ( control ) => {
									...
									switch ( control.type ) {
										...
										case 'select':
											...
											settings.options = control.options ? control.options : [];
											break;

										case 'number':
											component = 'TextControl';
											settings.type = 'number';
											settings.min  = control.min ? control.min : 1;
											settings.max  = control.max ? control.max : 100;
											settings.step = control.step ? control.step : 1;
											settings.onChange = function onChange( newValue ) {
												props.setAttributes( {
													[control.prop]: Number( newValue )
												} )
											};
											break;
										...
										case 'switcher':
											...
											settings.checked  = attributes[control.prop];
											settings.onChange = function onChange() {
												props.setAttributes( {
													[control.prop]: !attributes[control.prop]
												} )
											};
											break;
									...
									return <div key={ control.prop }>
										{ ( control.condition.indexOf( attributes.marker_label_format_cb ) >= 0 ) &&
											wp.element.createElement( wp.components[component], settings )
										}
									</div>;
								} ) }
